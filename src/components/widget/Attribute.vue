  ...
  <div class="widget attribute">
    <div v-show="selectNode">
      ...
      <div class="update" v-if="newVersion&&showUpdate">
        <div class="desc">
          <div>新版本：{{this.newNodeInfo.version}},当前版本：{{selectNode.version}}</div>
        </div>
              ...
              <div>
                <el-tooltip class="item" content="子组件如楼层般依次向下扩展" placement="top">
                  <span :style="{color: !selectNode.stack ? '#faad14' : 'inherit'}">
                    <i style="font-size: 12px" class="el-icon-info"></i>楼层模式
                  </span>
                </el-tooltip>
                <el-switch
                  key="1"
        ...
        <el-collapse-item title="属性设置" name="attr">
          ...
          >
            ...
            <template v-for="(item) in propsInfo">
              ...
              >
                <template>
                  <span class="label" v-if="item.label!='none'">
                  ...
                  <span class="value" v-if="item">
                    ...
                    <el-select
                      ...
                      v-model="selectNode.props[item.key]"
                      placeholder="请选择"
                      @change="renderData(selectNode.props[item.key],item.label)"
                    >
                      <el-option
...
import AttrEvent from "../attr/Event"; //方法
  ...
  computed: {
    newVersion: function() {
      if (
        this.selectNode &&
        this.newNodeInfo &&
        this.newNodeInfo.name == this.selectNode.type &&
        this.newNodeInfo.version != this.selectNode.version
      ) {
        return true;
      } else {
        return false;
  ...
  methods: {
    ...
    renderData: function(data, label) {
      if (label == "数据源") {
        ...
        let h = this.selectNode.style.height
        this.selectNode.style.width = h;
        this.selectNode.style.height = w;
      }
    },
    ...
    isCollapsed(attr) {
      ...
      return (
        activeNames &&
        ((activeNames.length && activeNames.indexOf(attr) > -1) ||
          activeNames == attr)
      );
    },
    checkNew: function({ type = "" } = {}) {
      ...
      var data = {
        ...
        pageSize: 5,
        terminalType: 1,
        token: "BC6B0FBE9E2D74F14F84",
        type: 1,
        authentication: "7e96afe3558ec4b2252fa266f22da051"
      };
