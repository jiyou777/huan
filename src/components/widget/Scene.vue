<template>
  <div class="appWarp dont-override" ref="appWarp" id="appWarp" @scroll="scrollChange" @mousemove="mousemove" @mouseup="mouseup" @mouseleave="mouseup" @click="bgClick">
    ...
    <div class="phoneSize">
      <div class="phoneItem" @click="changePhoneSize(item)" :class="{active: tvSize.name == item.name}" v-for="(item,index) in TV_SIZES" :key="index" :style="{width: parseInt(item.width)/5+'px', zIndex: 20 - index}">
        <div class="desc">{{item.name}} - {{item.width}}</div>
      </div>
    </div>
    <div class="app" id="stage" :class="[tvSize.name.toLowerCase().replace(/\s+/, '-')]" :style="{width: tvSize.width, minHeight: tvSize.height}">
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        tvSize: state => state.setting.tvSize,
        TV_SIZES: state => state.Metadata.tvSize
      }),
      isMainScene () {
    ...
    methods: {
      changePhoneSize (item) {
        this.$confirm(`确定要将画布切换为 ${parseInt(item.width)} x ${parseInt(item.height)} 吗？`)
          .then(() => {
            this.$store.dispatch('SettingChange', {tvSize: item})
