...
export default {
  ...
  methods: {
    ...
    initNodeEvent() {
      ...
      this.nodeInfo.events &&
        this.nodeInfo.events.forEach(element => {
          me.getComponent(me.nodeInfo.id, true).$el.addEventListener(
            element.event,
            event => {
              me.getComponent(
                me.nodeInfo.id,
                true
              ).oncallExecute(element.callList, [event]);
            }
          );
        });
    },
