    ...
    <div class="main-top-bar top-bar-lang-en">
      <div class="top-bar-left-section">
        <div class="top-bar-logo">
          <p style="color:#faad14">福建PROTAL</p>
        ...
        <div class="top-menus">
          <el-dropdown style="margin-left: 20px;" @command="selectLayout" trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-for="view in layouts" :key="view.type" :command="view.type">
                {{view.name}}
                <i
                  style="color: #faad14"
                  class="el-icon-check"
          ...
          <el-dropdown style="margin-left: 20px;" @command="selectWidget" trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item
                ...
                v-for="(view, name) in computedWidgets"
                :disabled="view.enable && view.required"
                :key="name"
                :command="name"
              >
                {{view.desc}}
                <i style="color: #faad14" class="el-icon-check" v-show="view.enable"></i>
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
...
export default {
  ...
  methods: {
    ...
    // 点击预览
    preview: function() {
      var urlInfo = common.parseURL('http://localhost:8565/html');
      var key = urlInfo.params && urlInfo.params.key;
      const open = () => {
        //const win = window.open(`http://localhost:8565/html`);

        // if (/desktop/i.test(this.Setting.tvSize.name)) {
      ...
      previewWindow.postMessage(
        {
          type: "preview",
          content: content,
          canvas: this.$store.state.setting.tvSize
        },
        "*"
      );
    }
