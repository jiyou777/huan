...
export default {
  ...
  computed: {
    fixedOrAbsolute() {
      if (
        this.info.position &&
        (this.info.position == "fixed" || this.info.position == "absolute")
      ) {
        return true;
      } else {
        return false;
    ...
    marginAuto: {
      get() {
        ...
        return (
          /^\s*auto/.test(style["margin-left"]) &&
          /^\s*auto/.test(style["margin-right"])
        );
      },
      set(val) {
        if (val) {
    ...
    boxShadowStr: {
      get() {
        return `${this.boxShadow.x}px ${this.boxShadow.y}px ${
          this.boxShadow.blur
        }px ${!this.boxShadow.spread ? "" : this.boxShadow.spread + "px"} ${this
          .boxShadow.color || ""} ${this.boxShadow.inset || ""}`.trim();
      },
      set(str = "") {
  ...
  watch: {
    ...
    info: {
      ...
      handler: function(newVal, oldVal) {
        ...
        this.showPosition =
          window.$vue && !window.$vue.isRootNode && this.fixedOrAbsolute;
