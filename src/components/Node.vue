...
export default {
    ...
    stacked: {
      handler(stacked) {
        ...
        if (styleShot.position == positionTypes[1]) {
          // 从备份获取 或者 生成样式；存备份
          this.nodeInfo.style = Object.assign(
            {},
            this.nodeInfo.style,
  ...
  computed: {
    ...
    tvSize() {
      return this.$store.state.setting.tvSize;
    },
    componentStyle() {
      return {
  ...
  methods: {
    ...
    async doLoad() {
      ...
      var style;
      this.checkstyle(that.info.style)
        if (this.checkstyle(that.info.style) && JSON.stringify(that.info.style) != "{}") {
          // 已经设置过样式了
      ...
      $props &&
        (Object.keys(that.nodeInfo.props) || []).forEach(key => {
    ...
    copyChild: function(node, { isJson, keepPos }) {
      ...
      for (let cloneNode of cloneNodes) {
        ...
        cloneNode = modifyNodeId(
          cloneNode,
          Object.keys(window.$_nodecomponents || {})
        );
