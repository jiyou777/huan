<template>
  <div v-if="visible" class="tool" :style="style">
    ...
    <template v-if="!isRootNode && stacked">
      <div class="item-h">
        <a class="item-sub" @click="moveLayer(1)">上移</a>
        <a class="item-sub" @click="moveLayer(-1)" v-show="!isRootNode">下移</a>
        <a class="item-sub" @click="moveLayer(Number.MAX_VALUE)" v-show="!isRootNode">置顶</a>
        <a class="item-sub" @click="moveLayer(Number.MIN_VALUE)" v-show="!isRootNode">置底</a>
      </div>
    </template>
    <template v-if="!isRootNode && !stacked && !fixed">
      <div class="item-h">
        <a class="item-sub" @click="moveLayer(1)">下移</a>
        <a class="item-sub" @click="moveLayer(-1)" v-show="!isRootNode">上移</a>
      </div>
    </template>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    fixed() {
      if (!this.node) return false;
      return (
        this.node.nodeInfo.style &&
        this.node.nodeInfo.style.position === "fixed"
      );
    }
  },
  mounted: function() {
    ...
    this.ema.bind("show.contextMenu", (node, e) => {
      ...
      this.$nextTick(() => {
        ...
        this.style.top =
          2 +
          pointerY -
          Math.max(menuHeight + 10 - (maxHeight - pointerY), 0) +
          "px";
      });
    });
  ...
  methods: {
    ...
    copy: function() {
      if (this.node && this.node.parentNodeVm) {
        console.log(this.node.parentNodeVm)
    ...
    reloadNode() {
      if (this.node)
        this.ema.fire("component.reload", this.node && this.node.nodeInfo.id);
    },
    importJson: function() {
      var me = this;
      var content = JSON.stringify(this.node.nodeInfo, null, 2);
      var reg = /(?:"id"\s?\:\s?")(\w+)"/;
      var idCache = (content.match(new RegExp(reg, "g")) || []).map(ids =>
        ids.replace(reg, (m, p) => p)
      );
      this.openDialog({
        name: "d-nodejson",
        data: {
    ...
    packNode() {
      typeof (this.node && this.node.doPack) === "function" &&
        this.node.doPack({ scene: "context_menu" });
    },
    unpackNode() {
      typeof (this.node && this.node.openPacked) === "function" &&
        this.node.openPacked({ scene: "context_menu" });
    }
  }
};
</script>
